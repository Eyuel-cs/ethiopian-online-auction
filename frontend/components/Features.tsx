...
export default function Features() {
  ...
  return (
    <div className="py-20 bg-slate-950 relative overflow-hidden">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[300px] bg-blue-600/10 blur-3xl rounded-full" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {features.map((feature) => (
            <div key={feature.titleKey}
              className={`relative group bg-gradient-to-br ${feature.bg} border border-white/5 rounded-2xl p-6 text-center overflow-hidden hover:scale-105 transition-all duration-300 cursor-default`}
              style={{boxShadow:'0 8px 32px rgba(0,0,0,0.4)'}}>
              <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-2xl"
                style={{background:`radial-gradient(circle at 50% 0%, ${feature.glow} 0%, transparent 70%)`}} />
              <div className={`relative w-16 h-16 mx-auto mb-5 rounded-2xl bg-gradient-to-br ${feature.gradient} flex items-center justify-center text-3xl shadow-lg`}
              ...
              <p className="text-slate-400 text-sm leading-relaxed relative z-10">{t(feature.descKey)}</p>
        ...
        <div className="mt-16 grid grid-cols-2 md:grid-cols-4 gap-6">
          {[
            ...
            <div key={stat.label} className="bg-white/5 border border-white/10 rounded-2xl p-5 text-center hover:bg-white/10 transition-all">
